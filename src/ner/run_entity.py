...
if __name__ == '__main__':
    ...
    parser.add_argument('--task', type=str, default=None, required=True, choices=['mimic01','mimic02','ace04', 'ace05', 'scierc'])
    ...
    parser.add_argument('--test_pred_filename', type=str, default="ent_pred_mimic_headct.json", help="the prediction filename for the test set")
    parser.add_argument('--train_data', type=str, default="./data/test.json", help="the prediction filename for the test set")
    parser.add_argument('--dev_data', type=str, default="./data/test.json", help="the prediction filename for the test set")
    parser.add_argument('--test_data', type=str, default="./data/test.json", help="the prediction filename for the test set")
    ...
    if args.do_train:
        ...
        print('LOAD DATA')
        train_data = Dataset(args.train_data,is_augment = args.do_aug)
        ...
        logger.info('eval_step=%d, train_batches=%d, eval_per_epoch=%d'%(eval_step,len(train_batches),args.eval_per_epoch))
    ...
    if args.do_eval:
        ...
        model = EntityModel(args, num_ner_labels=num_ner_labels)
        if args.eval_test:
            test_data = Dataset(args.test_data,is_augment = False)
            prediction_file = os.path.join(args.output_dir, args.test_pred_filename)
        else:
            test_data = Dataset(args.dev_data,is_augment = False)
