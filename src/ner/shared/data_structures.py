...
class Dataset:
    def __init__(self, json_file, pred_file=None, doc_range=None,is_augment=False):
...
class Document:
    ...
    def random_edit(self,word, insertion_prob=0.05, swap_prob=0.05, deletion_prob=0.05):
        ...
        if random.random() < insertion_prob:
            ...
            # 随机删除字符
            word_list = [char for char in word_list if random.random() > deletion_prob]
...
class NER:
    def __init__(self, ner, text, sentence_start, flavor=None):
        # print(ner[0], ner[1], text, sentence_start)
        self.span = Span(int(ner[0]), int(ner[1]), text, int(sentence_start))
        self.label = ner[2]
        self.flavor = flavor
...
def get_subtype_nerdict(ner):
    return_dict = {}
    for actual in ner:
        return_dict[actual.label] += 1
    return return_dict
...
def evaluate_sent(sent, counts):
    ...
    for actual in sent.ner:
        if actual.label == 'size':
            counts[f"concept_ner_gold"]  += 1
        else:
            counts[f"{actual.label}_ner_gold"]  += 1
    
    for prediction in sent.predicted_ner:
        if prediction.label == 'size':
            counts[f"concept_ner_predicted"]  += 1
        else:
            counts[f"{prediction.label}_ner_predicted"]  += 1
    
    for prediction in sent.predicted_ner:
        if any([prediction.span == actual.span for actual in sent.ner]):
            counts["notype_ner_matched"] += 1
        if any([prediction == actual for actual in sent.ner]):
            counts["strict_ner_matched"] += 1
            counts["ner_matched"] += 1
            correct_ner.add(prediction.span)
            if prediction.label == 'size':
                counts[f"concept_ner_matched"]  += 1
            else:
                counts[f"{prediction.label}_ner_matched"]  += 1
        else:
            ner_label = 0
            ner_type = None
            for actual in sent.ner:
                if actual.span.start_doc > prediction.span.end_doc or actual.span.end_doc < prediction.span.start_doc:
                    pass 
                else:
                    if prediction.label == 'size':
                        if actual.label  == 'concept':
                            ner_type = actual.label
                            ner_label = 1
                            break 
                    elif actual.label  == prediction.label:
                        ner_type = actual.label
                        ner_label = 1
                        break
            counts["ner_matched"] += ner_label
            if ner_type is not None:
                counts[f"{ner_type}_ner_matched"]  += 1
            correct_ner.add(prediction.span)
    ...
    for prediction in sent.predicted_relations:
        # print(prediction,sent.relations)
        if any([prediction.pair[0] == actual.pair[0] and prediction.pair[1] == actual.pair[1] for actual in sent.relations]):
            counts["notype_relations_matched"] += 1
...
def evaluate_predictions(dataset,ner_labels):
    counts = Counter()
    ...
    for ner_label in ner_labels:
        ner_label_scores = compute_f1(
        counts[f"{ner_label}_ner_predicted"], counts[f"{ner_label}_ner_gold"], counts[f"{ner_label}_ner_matched"])
        ner_label_dict[ner_label] = ner_label_scores

    scores_ner = compute_f1(
        counts["ner_predicted"], counts["ner_gold"], counts["ner_matched"])
    scores_strict_ner = compute_f1(
        counts["ner_predicted"], counts["ner_gold"], counts["strict_ner_matched"])
    scores_notype_ner = compute_f1(
        counts["ner_predicted"], counts["ner_gold"], counts["notype_ner_matched"])
    scores_relations = compute_f1(
        counts["relations_predicted"], counts["relations_gold"], counts["relations_matched"])
    scores_notype_relations = compute_f1(
        counts["relations_predicted"], counts["relations_gold"], counts["notype_relations_matched"])
    scores_strict_relations = compute_f1(
        counts["relations_predicted"], counts["relations_gold"], counts["strict_relations_matched"])

    return dict(ner=scores_ner, strict_ner = scores_strict_ner, notype_ner = scores_notype_ner,relation=scores_relations, strict_relation=scores_strict_relations,notype_relation = scores_notype_relations),ner_label_dict
